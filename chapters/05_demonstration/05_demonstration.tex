...
Thresholds must therefore be calibrated as a function of selectivity, denoted by \(\tau(s)\).
...
\paragraph{Why all of this, if we already have correlated and uncorrelated workloads defined in Section~\ref{sec:query-generation}?}
...
The null distribution of these heuristic scores is difficult to write down
analytically because it depends on the empirical metadata distribution, the
cluster assignment of sampled queries, the histogram binning, and the random
range construction. Therefore, we estimate it by Monte Carlo simulation.
Let 
\[
c(q,\phi)
\]
