...
For $\mathrm{corr}_1$, the score decomposes as:
\[
\mathrm{corr}_1(q) = \mathrm{JS}(P_{c^*} \,\|\, P_{\mathrm{GLOBAL}}) \cdot \mathrm{sign}(\mathrm{lift}(q)).
\]
...
Thus, $\mathrm{corr}_1$ captures both the strength of distributional deviation and the direction of alignment.

\paragraph{Tanh-scaled Interpretation.}
For $\mathrm{corr}_2$, the score is given by:
\[
\mathrm{corr}_2(q) = \mathrm{JS}(P_{c^*} \,\|\, P_{\mathrm{GLOBAL}}) \cdot \tanh(\mathrm{lift}(q)).
\]
...
If $\mathrm{JS}(P_{c^*} \,\|\, P_{\mathrm{GLOBAL}}) \approx 0$, then $\mathrm{corr}_2(q) \approx 0$, regardless of the lift.

Thus, $\mathrm{corr}_2$ reflects both distributional deviation and the strength of alignment in a continuous manner.

\paragraph{Difference-based Interpretation.}
For $\mathrm{corr}_3$, the score is defined as:
\[
\mathrm{corr}_3(q) = \mathrm{JS}(P_{c^*} \,\|\, P_{\mathrm{GLOBAL}}) \cdot \big(P_{c^*}([L,U]) - P_{\mathrm{GLOBAL}}([L,U])\big).
\]
...
If $\mathrm{JS}(P_{c^*} \,\|\, P_{\mathrm{GLOBAL}}) \approx 0$, then $\mathrm{corr}_3(q) \approx 0$, regardless of the difference term.

Thus, $\mathrm{corr}_3$ combines distributional deviation with absolute, rather than relative, alignment effects.


\section{Thresholding with \texorpdfstring{$\tau$}{tau}}
...
\subsection{Selectivity Dependence.}
